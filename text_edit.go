//source: https://github.com/therecipe/qt/tree/master/internal/examples/widgets/textedit
...
func newTextEditExample(qApp *widgets.QApplication) *widgets.QWidget {
	...
	if len(os.Getenv("QT_NO_CLIPBOARD")) != 0 {
		...
		gui.QGuiApplication_Clipboard().ConnectDataChanged(this.clipboardDataChanged)
	}

	textEdit.SetFocus2()
	this.setCurrentFileName("")

	this.load(":/qml/example.html")

	return this.QWidget_PTR()
}
...
func (t *TextEdit) setupTextActions() {
	...
	if gui.QGuiApplication_IsLeftToRight() {
